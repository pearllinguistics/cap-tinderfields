...
Capistrano::Configuration.instance.load do |instance|
  after "deploy:setup",           "deploy:db:setup"   unless fetch(:skip_db_setup, false) || fetch(:no_rails, false)
  after "deploy:finalize_update", "deploy:db:symlink" unless fetch(:no_rails, false)
  
  set :template_dir, "#{File.dirname(__FILE__)}/templates"
  ...
  namespace :deploy do
    ...
    namespace :db do
      ...
      task :setup, :except => { :no_release => true } do
        ...
        puts fetch(:template_dir, "config/deploy")

        
        location = fetch(:template_dir, "config/deploy") + '/database.yml.erb'
        template = File.file?(location) ? File.read(location) : default_template
        puts location
        puts File.file?(location)
